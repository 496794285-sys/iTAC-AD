...
def main():
    device = "cpu"
    model = ITAC_AD(feats=7, d_model=128, n_heads=8, e_layers=2, dropout=0.1).to(device)
    aac = AACScheduler(window_size=256, quantile_p=0.9)

    # 先进行一次前向传播来初始化 lazy 参数
    dummy_input = torch.randn(1, 96, 7).to(device)
    with torch.no_grad():
        _ = model(dummy_input)
